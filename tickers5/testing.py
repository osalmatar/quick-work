...
def transfer_records():
    ...
    # Step 1: Retrieve Buy Price for each Ticker before deletion
    buy_price_query = '''
    SELECT "Ticker", "Buy Price" 
    FROM trade_journal
    WHERE "Buy Price" IS NOT NULL
    '''
    buy_price_df = pd.read_sql(buy_price_query, conn)

    # Convert buy_price_df to dictionary for easy lookup
    buy_price_dict = buy_price_df.set_index('Ticker')['Buy Price'].to_dict()

    # Step 2: Fetch closed records to transfer
    closed_records_query = '''
    SELECT 
        "Buy Date", "Ticker", "Buy Strat", "Last Alert Date", "ll", 
        "Buy Price", "Target", "Call", "Status", "CMP", "CMP_Date", 
    ...
    # Step 3: Fill missing Buy Price values in closed_records_df using buy_price_dict
    closed_records_df['Buy Price'] = closed_records_df.apply(
        lambda row: buy_price_dict.get(row['Ticker'], row['Buy Price']) 
        if pd.isna(row['Buy Price']) else row['Buy Price'], axis=1
    )
