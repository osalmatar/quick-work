...
def save_and_upload(final_table):
    ...
    final_table['Quantity'] = pd.to_numeric(final_table['Quantity'], errors='coerce')
    final_table['Sell Price'] = pd.to_numeric(final_table['Sell Price'], errors='coerce')

    final_table['Buy Date'] = pd.to_datetime(final_table['Buy Date']).dt.date
    final_table['Last Alert Date'] = pd.to_datetime(final_table['Last Alert Date']).dt.date
    ...
    final_table.to_csv('final_trade_journal.csv', index=False)
...
def main():
    ...
    final_table = fetch_and_merge_cmp_and_ew_conv(final_table)
    save_and_upload(final_table)
    transfer_records()
