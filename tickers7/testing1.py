...
def transfer_reversed():
    ...
    # Fetch CMP and CMP_Date from yfinance_data and merge with records_df
    cmp_query = '''
    SELECT "Ticker", "CMP", "CMP_Date"
    FROM yfinance_data;
    '''
    cmp_df = pd.read_sql(cmp_query, conn)

    # Merge CMP and CMP_Date into records_df based on Ticker
    records_df = pd.merge(records_df, cmp_df, on='Ticker', how='left')

    # Handle missing CMP or CMP_Date values if needed
    records_df['CMP'] = records_df['CMP'].fillna(0)  # Default or handle missing values as appropriate
    records_df['CMP_Date'] = pd.to_datetime(records_df['CMP_Date'], errors='coerce')

    # Set Status and Call fields to default "Open" and "Long"
    records_df['Status'] = 'Open'
    records_df['Call'] = 'Long'

    # Recalculate Risk Amount and Target
    ...
    for _, row in records_df.iterrows():
        ...
        INSERT INTO reversed_table (
            ...
            "Sell Price", "Broker", "CMP", "CMP_Date")
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT ("Buy Date", "Ticker")
        DO NOTHING;
        '''
