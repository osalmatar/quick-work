...
def save_and_upload(merged_table):
    ...
    for _, row in merged_table.iterrows():
        # Insert or update for composite key ("Stock", "Buy Date")
        insert_query_composite = '''
        INSERT INTO TJ ("Trade Day", "System", "Trade Direction", "Stock", "Entry Price", "ISL/TSL", "Buy Qty", 
        "Sell Qty", "Sell Price", "Exit Strategy", "Commission", "Trade Category", "Account", "CMP", "CMP_Date") 
...
def update_table_with_adjusted_status(table_name, conn):
    ...
    SELECT 
        ...
        "Commission", "Trade Category", "Account", "CMP", "CMP_Date"
    FROM {table_name};
    '''
    ...
    def adjust_call_status(row):
        # Logic for TJ
        if pd.notnull(row['CMP']) and pd.notnull(row['ISL/TSL']) and row['CMP'] < row['ll']:
            return 'Short', 'ISL Hit', round(row['ISL/TSL'], 2)
        elif pd.notnull(row['CMP']) and pd.notnull(row['Target']) and row['CMP'] <= row['Target']:
            return 'Short', 'Target Hit', round(row['Target'], 2)
...
def transfer_closed():
    ...
    SELECT 
        ...
        "Trade Category", "Account", "CMP", "CMP_Date"
    FROM TJ
...
def fetch_and_export_to_excel():
    ...
    # Fetch Ticker from TJ
    tj_ticker_query = ''' SELECT "Stock", "Account" FROM TJ'''
    ...
    merge_buy = pd.merge(ew_conv_buy_df, tj_ticker_df,
    left_on='Ticker', right_on='Stock', how='left')
    ...
    merge_sell = pd.merge(ew_conv_sell_df, tj_ticker_df,
    left_on='Ticker', right_on='Stock', how='left')
