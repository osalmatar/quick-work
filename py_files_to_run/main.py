...
def fetch_and_merge_from_postgresql(final_table):
    ...
    ew_conv_query = '''
    SELECT "Ticker", "Last Alert Date", "Target", "Call"
    FROM ew_conv_table
    '''
    ...
    # Check if CMP < ISL to hit ISL
        ...
        # Check if CMP <= Target to hit Target from ew_conv_table
            ...
            return 'Short', 'Target Hit', round(row['CMP'], 2)
...
def save_and_upload(merged_table):
    ...
    merged_table['Last Alert Date'] = merged_table['Last Alert Date'].apply(lambda x: None if pd.isna(x) else x)
    ...
    merged_table = merged_table[['Trade Day', 'System', 'Trade Direction', 'Stock', 'Entry Price', 'ISL/TSL', 
                               'Buy Qty', 'Sell Qty', 'Sell Price', 'Exit Strategy', 'Commission', 'Trade Category',
                                 'Account', 'CMP', 'CMP_Date', 'Target', 'Last Alert Date']]
    ...
    for _, row in merged_table.iterrows():
        ...
        "Sell Qty", "Sell Price", "Exit Strategy", "Commission", "Trade Category", "Account", "CMP", "CMP_Date", "Target", "Last Alert Date") 
        ...
        cur.execute(insert_query_composite, (
            ...
            row["Commission"], row["Trade Category"], row["Account"], row["CMP"], row["CMP_Date"], row["Target"], row["Last Alert Date"]
    ))
...
def update_table_with_adjusted_status(table_name, conn):
    ...
    SELECT 
        ...
        "Commission", "Trade Category", "Account", "CMP", "CMP_Date", "Target", "Last Alert Date"
    FROM {table_name};
    '''
...
def transfer_closed():
    ...
    SELECT 
        ...
        "Trade Category", "Account", "CMP", "CMP_Date", "Target", "Last Alert Date"
    FROM tj
